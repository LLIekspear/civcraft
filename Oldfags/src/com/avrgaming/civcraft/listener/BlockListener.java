...
import static com.avrgaming.civcraft.util.CivColor.Red;
...
import ru.constellation.ellidey.packets.C0PacketBottomString;
...
//		}
	...
	public void blockPlace(BlockPlaceEvent event){
		if (ItemManager.getId(event.getBlockPlaced()) == CivData.WOOD_DOOR ||
				ItemManager.getId(event.getBlockPlaced()) == CivData.IRON_DOOR||
				ItemManager.getId(event.getBlockPlaced()) == CivData.SPRUCE_DOOR||
				ItemManager.getId(event.getBlockPlaced()) == CivData.BIRCH_DOOR||
				ItemManager.getId(event.getBlockPlaced()) == CivData.JUNGLE_DOOR||
				ItemManager.getId(event.getBlockPlaced()) == CivData.ACACIA_DOOR||
				ItemManager.getId(event.getBlockPlaced()) == CivData.DARK_OAK_DOOR) {
			Buildable b = CivGlobal.getNearestBuildable(event.getBlockPlaced().getLocation());
		if(b.getConfigId().equals("ti_trade_outpost") || b.getConfigId().equals("ti_fishing_boat")){
			if(b.getCenterLocation().getLocation().getChunk().equals(event.getBlockPlaced().getLocation().getChunk())){
				event.setCancelled(true);
				CivMessage.sendRightChat(event.getPlayer(), CivColor.Red + "trade_warning_door");
			}
		}
		}
	} 
