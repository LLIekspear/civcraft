		...
		if (perkList.size() != 0 || personalUnboundPerks.size() != 0) {
			...
			for (Perk perk : perkList) {
				infoRec = LoreGuiItem.build(perk.getDisplayName(), 
						...
						perk.configPerk.data, CivColor.Gold+"<������ ��� ���������>",
...
//		}
	...
	public void onDamageNotification(Player player, BuildableDamageBlock hit) {
		...
		CivMessage.sendTownRightMessage(hit.getTown(), CivColor.Yellow+CivSettings.localize.localizedString("var_buildable_underAttackAlert",hit.getOwner().getDisplayName(),hit.getOwner().getCorner(),hit.getOwner().getDamagePercentage()));	
	}
...
//	}
	...
	public void onInvalidPunish() {
		...
		DecimalFormat df = new DecimalFormat("###");
		CivMessage.sendTownRightMessage(this.getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_buildable_cannotSupport",this.getDisplayName(),(center.getX()+","+center.getY()+","+center.getZ())));
		CivMessage.sendTownRightMessage(this.getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_buildable_cannotSupportDamage",df.format(invalid_hourly_penalty*100),(this.hitpoints+"/"+this.getMaxHitPoints())));
		CivMessage.sendTownRightMessage(this.getTown(), CivColor.Rose+this.invalidLayerMessage);
		CivMessage.sendTownRightMessage(this.getTown(), CivColor.Rose+CivSettings.localize.localizedString("buildable_validationPrompt"));
		this.save();
