...
public class BuildAsyncTask extends CivAsyncTask {
		...
		for (; buildable.getBuiltBlockCount() < (tpl.size_x*tpl.size_y*tpl.size_z); buildable.builtBlockCount++) {
			...
			if (buildable instanceof Wonder) {
				if (buildable.getTown().getMotherCiv() != null) {
					CivMessage.sendTownRightMessage(buildable.getTown(), CivSettings.localize.localizedString("var_buildAsync_wonderHaltedConquered",buildable.getTown().getCiv().getName()));
					try {
				...
				if (inProgress != null && inProgress != buildable) {
					CivMessage.sendTownRightMessage(buildable.getTown(), CivSettings.localize.localizedString("var_buildAsync_wonderHaltedOtherConstruction",inProgress.getDisplayName()));
					try {
				...
				if (buildable.getTown().getTownHall() == null) {
					CivMessage.sendTownRightMessage(buildable.getTown(), CivSettings.localize.localizedString("buildAsync_wonderHaltedNoTownHall"));
					try {
			...
			try {
				...
				if (nextPercentComplete > this.percent_complete) {
					...
					if ((this.percent_complete % 10 == 0)) {
						if (buildable instanceof Wonder) {
							...
							CivMessage.sendTownRightMessage(buildable.getTown(),
				...
				if (buildable instanceof Wonder) {
					...
					if (buildable.isDestroyed()) {
						CivMessage.sendTownRightMessage(buildable.getTown(), CivSettings.localize.localizedString("var_buildAsync_destroyed",buildable.getDisplayName()));
						abortWonder();
						return false;
					}
	...
	private void processWonderAbort() {
		CivMessage.sendTownRightMessage(buildable.getTown(), CivColor.Rose+CivSettings.localize.localizedString("var_buildAsync_wonderFarAway",buildable.getDisplayName()));
		
		//Refund the town half the cost of the wonder.
		double refund = (int)(buildable.getCost() / 2);			
		buildable.getTown().depositDirect(refund);

		CivMessage.sendTownRightMessage(buildable.getTown(), CivColor.Yellow+CivSettings.localize.localizedString("var_buildAsync_wonderRefund",refund,CivSettings.CURRENCY_NAME));
		abortWonder();
	}
