...
public class EffectEventTimer extends CivAsyncTask {
	...
	private void processTick() {
		...
		for (Town town : CivGlobal.getTowns()) {
			...
			if (townhall == null) {
				CivMessage.sendTownRightMessage(town, CivColor.Yellow+CivSettings.localize.localizedString("effectEvent_noTownHall"));
				continue;
			}
			...
			try {
				...
				if (unusedBeakers > 0) {
					...
					if (cultureFromBeakers > 0) {
						CivMessage.sendTownRightMessage(town, CivColor.LightGreen+CivSettings.localize.localizedString("var_effectEvent_convertBeakers",(CivColor.LightPurple+
			...
			cultureGenerated = Math.round(cultureGenerated);
			CivMessage.sendTownRightChat(town, CivColor.LightGreen+CivSettings.localize.localizedString("var_effectEvent_generatedCulture",(CivColor.LightPurple+cultureGenerated+CivColor.LightGreen)));
		}
