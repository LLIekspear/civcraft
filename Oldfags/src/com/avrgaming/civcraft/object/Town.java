...
//		}
	...
	public AttrSource getCulture() {
		...
		if (this.getBuffManager().hasBuff("buff_globe_theatre_culture_from_towns")) {
			...
			CivMessage.sendTownRightMessage(this, CivColor.LightGreen+CivSettings.localize.localizedString("var_town_GlobeTheatreCulture",CivColor.Yellow+bonus+CivColor.LightGreen,townCount));
	...
	public void addAccumulatedCulture(double generated) {
		...
		if (this.getCultureLevel() != CivSettings.getMaxCultureLevel()) {
			if (this.culture >= clc.amount) {
				...
				CivMessage.sendCivRightMessage(this.civ, CivSettings.localize.localizedString("var_town_bordersExpanded",this.getName()));
			}
		}
		return;
	}
	...
	public AttrSource getHammers() {
        ...
        double mines = 0;
        for (Structure struct : this.structures.values()) {
            if (struct != null && (struct instanceof Mine)) {
...
//		return this.assistantGroup.getName();	
	...
	public void depositTaxed(double amount) {
		...
		if (this.getMotherCiv() != null) {
			...
			CivMessage.sendTownRightMessage(this, CivColor.Yellow+CivSettings.localize.localizedString("var_town_capturePenalty1",(amount - capturePayment),CivSettings.CURRENCY_NAME,this.getCiv().getName()));
	...
	public void buildWonder(Player player, String id, Location center, Template tpl) throws CivException {
		...
		CivMessage.sendTownRightMessage(this, CivColor.Yellow+CivSettings.localize.localizedString("var_town_buildwonder_success",wonder.getDisplayName()));
		this.save();
	}
	 ...
	 public void buildStructure(Player player, String id, Location center, Template tpl) throws CivException {
	        ...
	        this.getTreasury().withdraw(cost);
	        CivMessage.sendTownRightMessage(this, CivColor.Yellow + CivSettings.localize.localizedString("var_town_buildStructure_success", struct.getDisplayName()));
	...
	public boolean processSpyExposure(Resident resident) {
		...
		try {
			...
			if (percent >= CivSettings.getDouble(CivSettings.espionageConfig, "espionage.town_exposure_failure")) {
				failed = true;
				CivMessage.sendTownRightMessage(this, CivColor.Yellow+CivColor.BOLD+CivSettings.localize.localizedString("town_spy_thwarted"));
				return failed;
			} 
			...
			if (message.length() > 0) {
				if (lastMessage == null || !lastMessage.equals(message)) {
					CivMessage.sendTownRightMessage(this, CivColor.Yellow+CivColor.BOLD+message);
					lastMessage = message;
				}
			}
		} catch (InvalidConfiguration e) {
